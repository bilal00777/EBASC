...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
        ...
        if ($image['error'] === UPLOAD_ERR_OK) {
            ...
            if (in_array($imageFileType, $validTypes) && $image['size'] <= $maxSize) {
                ...
                if (move_uploaded_file($image['tmp_name'], $target_file)) {
                    try {
                        $stmt = $pdo->prepare("INSERT INTO photogallerytwo (title, image_path) VALUES (:title, :image_path)");
    ...
    if (isset($_POST['delete'])) {
        $id = $_POST['id'];
        try {
            $stmt = $pdo->prepare("DELETE FROM photogallerytwo WHERE id = :id");
...
$query = $pdo->query("SELECT id, title, image_path FROM photogallerytwo ORDER BY id DESC");
...
<br><br><br>
<hr>
<h1 style=" text-transform: uppercase;"> uploaded images</h1>
<br>
 ...
 <div class="row">
        <?php foreach ($records as $record): ?>
            <div class="col-md-6">
                <div class="card mb-4">
                    <img src="uploads_photo_g/<?php echo htmlspecialchars($record['image_path']); ?>" class="card-img-top" style="width: 100%; height: 200px; object-fit: cover;">
                    <div class="card-body">
